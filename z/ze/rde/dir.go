...
func (aa *DirRouter) Handle(method, action string, handle z.HandleFunc) {
	...
	if z.G.Server.ApiRoot != "" {
		action = strings.TrimPrefix("/"+action, z.G.Server.ApiRoot+"/")
	}
...
func (aa *DirRouter) ServeHTTP(rw http.ResponseWriter, rr *http.Request) {
	...
	if z.G.Server.ApiRoot != "" {
		rr.URL.Path = strings.TrimPrefix(rr.URL.Path, z.G.Server.ApiRoot+"/")
		rr.URL.RawPath = strings.TrimPrefix(rr.URL.RawPath, z.G.Server.ApiRoot+"/")
	}
