// at https://github.com/jinzhu/copier/blob/master/License.
...
var (
	...
	ErrFieldNameTagStartNotUpperCase = errors.New("field name tag start must be upper case")
)
...
type Option struct {
	...
	NoPanic bool
	// TagName is the name of the tag to use for copying. Defaults to "cpy"
	TagName string
}
...
func Copy(toValue any, fromValue any) (err error) {
	return copier(toValue, fromValue, Option{TagName: "cpy"})
}

// CopyWith copy with option
func CopyWith(toValue any, fromValue any, opt Option) (err error) {
	if opt.TagName == "" {
		opt.TagName = "cpy"
	}
		...
		if len(converters) > 0 {
			if ok, e := set(dest, source, opt.DeepCopy, converters); e == nil && ok {
				if isSlice {
					...
					if to.Type().Elem().Kind() == reflect.Pointer {
...
func set(to, from reflect.Value, deepCopy bool, converters map[converterPair]TypeConverter) (bool, error) {
	...
	if to.Kind() == reflect.Pointer {
		...
		} else if from.Kind() != reflect.Pointer && from.IsZero() {
			to.Set(reflect.Zero(to.Type()))
			return true, nil
		}
		// depointer `to`
...
func getFlags(dest, src reflect.Value, toType, fromType reflect.Type, opt Option) (flags, error) {
	...
	for _, field := range toTypeFields {
		tags := field.Tag.Get(opt.TagName)
		if tags != "" {
			var name string
			var err error
