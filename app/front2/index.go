...
// IndexWithTRY，依赖FileFS，不支持文件变动
func (aa *FrontHandler) IndexWithTRY(rw http.ResponseWriter, rr *http.Request, rp string) {
	fpath := rr.URL.Path
	if fpath == "" {
		fpath = "/"
	}
...
// IndexWithCHG, 不依赖FileFS，支持文件变动
func (aa *FrontHandler) IndexWithCHG(rw http.ResponseWriter, rr *http.Request, rp string) {
	redirect := false
